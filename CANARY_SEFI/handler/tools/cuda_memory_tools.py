...
def check_cuda_memory_alloc_status(empty_cache=False, show_status=False):
    if torch.cuda.is_available():
        ...
        if empty_cache:
            reporter.console_log("[CUDA-REPORT] CUDA Cache being released", Fore.CYAN, type="DEBUG", show_batch=False, show_step_sequence=False)
            torch.cuda.empty_cache()
            if show_status:
                show_cuda_memory_alloc_status()
    else:
        reporter.console_log("[CUDA-REPORT] CUDA is not AVAILABLE", Fore.RED, show_batch=False, show_step_sequence=False)
...
def show_cuda_memory_alloc_status():
    ...
    reporter.console_log(log, Fore.GREEN, type="DEBUG", show_batch=False, show_step_sequence=False)