...
# 新增对抗样本转换文件记录
def add_adv_trans_img_file_log(trans_id, attack_id, adv_img_file_id, adv_trans_img_filename, ground_valid=None):
    sql_insert = "INSERT INTO adv_trans_img_file_log (adv_trans_img_file_id, trans_id, attack_id, adv_img_file_id, adv_trans_img_filename, ground_valid) " \
          "VALUES (NULL,?,?,?,?,?)"

    sql_query = " SELECT adv_trans_img_file_id FROM adv_trans_img_file_log " \
                " WHERE trans_id = ? AND attack_id = ? AND adv_img_file_id = ? AND adv_trans_img_filename = ? AND ground_valid = ?"
    args = (trans_id, attack_id, adv_img_file_id, adv_trans_img_filename, ground_valid)
    ...
    if task_manager.test_data_logger.debug_log:
        msg = "[ LOGGER ] Logged. Adv_Trans file ID: {}. Trans Img file name: {}. Adv_Img file ID: {}."\
            .format(adv_trans_img_file_id, adv_trans_img_filename, adv_img_file_id)
...
def set_adv_trans_file_ground_valid(adv_trans_img_file_id, ground_valid):
    ...
    if task_manager.test_data_logger.debug_log:
        msg = "[ LOGGER ] Logged. Adv_trans file ID: {}. Ground Valid: {}.".format(adv_trans_img_file_log, ground_valid)
        reporter.console_log(msg, Fore.CYAN, type="DEBUG")
...
def find_adv_trans_file_logs_by_attack_id_and_trans_id(attack_id, trans_id):
    sql = "SELECT * FROM adv_trans_img_file_log WHERE attack_id = ? AND trans_id = ? "
    return task_manager.test_data_logger.query_logs(sql, (attack_id, trans_id))
