...
def add_attack_log(atk_name, base_model, ori_img_id, adv_img_filename):
    sql = "INSERT INTO adv_example_log (adv_img_id,batch_id,atk_name,base_model,ori_img_id,adv_img_filename) " \
          "VALUES (NULL, '{}', '{}', '{}', '{}', '{}')" \
        .format(str(batch_flag.batch_id), str(atk_name), str(base_model), str(ori_img_id), str(adv_img_filename))

    adv_img_id = log.insert_log(sql)
    if log.debug_log:
        print(Fore.CYAN + "[LOGGER] 已写入日志 本次攻击生成adv_img_id为 {} 存储文件名{}".format(adv_img_id, adv_img_filename))
...
def add_adv_build_log(adv_img_id, cost_time):
    ...
    log.update_log(sql)
    if log.debug_log:
        print(Fore.CYAN + "[LOGGER] 已写入日志 adv_img_id为 {} 的对抗样本耗时{}".format(adv_img_id, cost_time))
...
def add_adv_da_log(adv_img_id, adv_da_test_result):
    ...
    log.update_log(sql)
    if log.debug_log:
        print(Fore.CYAN + "[LOGGER] 已写入日志 adv_img_id为{}的对抗样本 "
