...
def save_trans_deflection_capability_indicator_data(atk_name, base_model, trans_name, inference_model, trans_file_type,
                                                    MR, AIAC, ARTC, ACAMC_A, ACAMC_T, atk_perturbation_budget=None, logger=None):
    if logger is None:
        logger = task_manager.test_data_logger
    sql = "REPLACE INTO trans_deflection_capability_indicator_data (atk_name, base_model, atk_perturbation_budget, " \
          "trans_name, inference_model, trans_file_type, MR, AIAC, ARTC, ACAMC_A, ACAMC_T) VALUES (?,?,?,?,?,?,?,?,?,?,?)"
    args = (str(atk_name), str(base_model), atk_perturbation_budget, trans_name, str(inference_model), str(trans_file_type),
            MR, AIAC, ARTC, ACAMC_A, ACAMC_T)
    logger.insert_log(sql, args)
    if logger.debug_log:
        msg = "[ LOGGER ] Logged. Tested Trans File is generated by Attack Method({} *BaseModel {}* " \
              "*PerturbationBudget {}*) *Trans name {}*). Tested Trans file is inferenced by Model({}). " \
              "Test trans file type:{}. Trans Method Capability Indicator is " \
              "[ MR:{} AIAC:{} ARTC:{} ACAMC_A:{} ACAMC_T:{} ]".format(*args)
        reporter.console_log(msg, Fore.CYAN, type="DEBUG")
    return
