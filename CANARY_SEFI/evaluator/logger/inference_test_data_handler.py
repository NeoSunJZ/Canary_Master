...
def save_inference_test_data(img_id, img_type, inference_model, inference_img_label, inference_img_conf_array):
    ...
    inference_test_data_id = logger.insert_log(sql, args)
    if logger.debug_log:
...
def get_all_inference_test_data(img_type):
    ...
    return handle_result(logger.query_logs(sql, (str(img_type), )))
...
def get_inference_test_data_by_model_name(model_name, img_type):
    ...
    return handle_result(logger.query_logs(sql, (str(img_type), str(model_name))))


def get_inference_test_data_by_img_id(img_id, img_type):
    sql = "SELECT * FROM inference_test_data WHERE img_type = ? AND img_id = ?"
    return handle_result(logger.query_logs(sql, (str(img_type), img_id)))
...
def get_clean_inference_test_data_with_img_info(inference_model):
    ...
    return handle_result(logger.query_logs(sql, (str(inference_model),)))
...
def get_adv_inference_test_data_with_adv_info(inference_model, adv_example_file_type='ADVERSARIAL_EXAMPLE'):
    sql = "SELECT inference_test_data.*, ori_img_log.ori_img_label, ori_img_log.ori_img_id, " \
          ...
          "AND inference_test_data.inference_model = ? "
    return handle_result(logger.query_logs(sql, (str(adv_example_file_type),str(inference_model))))
