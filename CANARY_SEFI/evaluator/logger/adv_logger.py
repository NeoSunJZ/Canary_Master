...
def add_adv_base_log(attack_id, ori_img_id, adv_img_filename):
    sql = "INSERT INTO adv_example_log (adv_img_id,batch_id,attack_id,ori_img_id,adv_img_filename) " \
          "VALUES (NULL,?,?,?,?)"
    adv_img_id = log.insert_log(sql, (str(batch_flag.batch_id), str(attack_id), str(ori_img_id), str(adv_img_filename)))
    if log.debug_log:
...
def add_adv_build_log(adv_img_id, cost_time):
    sql = "UPDATE adv_example_log SET cost_time = ? WHERE adv_img_id = ?"
    log.update_log(sql, (str(cost_time), str(adv_img_id)))
    if log.debug_log:
...
def add_adv_da_log(adv_img_id, adv_da_test_result):
    sql = "UPDATE adv_example_log " \
          ...
          "WHERE adv_img_id = ?"
    log.update_log(sql, (str(adv_da_test_result.get("maximum_disturbance")), str(adv_da_test_result.get("euclidean_distortion")),
...
def find_all_adv_log(batch_id):
    sql = " SELECT * FROM adv_example_log WHERE batch_id = ?"
    return log.query_logs(sql, (str(batch_id)))


def find_adv_log_by_attack_id(batch_id, attack_id):
    sql = " SELECT * FROM adv_example_log WHERE batch_id = ? and attack_id = ?"
    return log.query_logs(sql, (str(batch_id), attack_id))


def find_adv_log_by_img_id(adv_img_id):
    sql = " SELECT * FROM adv_example_log WHERE adv_img_id = ? "
    return log.query_log(sql, (adv_img_id,))
