...
# 生成对抗样本
def adv_example_generate(dataset_info, attacker_list, attacker_config, model_config, img_proc_config, adv_example_generate_batch_config):
    # 标记当前步骤
    ...
    def function(atk_name, atk_args, model_name, model_args, img_proc_args, run_device):
        msg = "[Device {}] Generating Adv Example By Attack Method {} on(base) Model {}.".format(run_device, atk_name, model_name)
...
def attack_adv_example_da_test(attacker_list, dataset_info, use_raw_nparray_data=False):
    # 标记当前步骤
    task_manager.sys_log_logger.set_step(Step.ATTACK_ADV_EXAMPLE_DA_TEST)
    for atk_name in attacker_list:
        ...
        for attack_log in attack_logs:
            ...
            msg = "Test Adv Example(Generated by Attack Method({} *Base Model {}*)" \
                  "(Use Numpy Array Data:{}))'s Disturbance-aware" \
...
def attack_adv_example_da_evaluation(attacker_list, use_raw_nparray_data=False):
    # 标记当前步骤
    task_manager.sys_log_logger.set_step(Step.ATTACK_ADV_EXAMPLE_DA_EVALUATION)
    for atk_name in attacker_list:
    ...
    for atk_name in attacker_list:
        for base_model in attacker_list[atk_name]:
            ...
            def function(model_name, model_args, img_proc_args, run_device):
                ...
                adv_inference(attack_log, model_name, model_args, img_proc_args, use_raw_nparray_data, run_device=run_device)
...
def model_inference_capability_test(dataset_info, model_list, model_config, img_proc_config, inference_batch_config):
    ...
    def function(model_name, model_args, img_proc_args, run_device):
        ...
        reporter.console_log(msg, Fore.GREEN, show_task=True, show_step_sequence=True)
        inference(dataset_info, model_name, model_args, img_proc_args, inference_batch_config, run_device=run_device)
...
def adv_example_generate_with_perturbation_increment(dataset_info, attacker_list, attacker_config, model_config,
                                                     img_proc_config, adv_example_generate_batch_config, perturbation_increment_config):
    # 标记当前步骤
    task_manager.sys_log_logger.set_step(Step.ADV_EXAMPLE_GENERATE_WITH_PERTURBATION_INCREMENT)

    def function(atk_name, atk_args, model_name, model_args, img_proc_args, run_device):
        msg = "Generating Adv Example By Attack Method {} on(base) Model {} in the case of increasing perturbations.".format(atk_name, model_name)
        reporter.console_log(msg, Fore.GREEN, show_task=True, show_step_sequence=True)
...
# 攻击偏转能力测试(扰动递增)
def attack_deflection_capability_test_with_perturbation_increment(attacker_list, model_config, img_proc_config, use_raw_nparray_data=False):
    # 标记当前步骤
...
def attack_adv_example_da_test_with_perturbation_increment(attacker_list, dataset_info, use_raw_nparray_data=False):
    # 标记当前步骤
    task_manager.sys_log_logger.set_step(Step.ATTACK_ADV_EXAMPLE_DA_TEST_WITH_PERTURBATION_INCREMENT)
