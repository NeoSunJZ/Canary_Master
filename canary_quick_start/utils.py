...
def load_test_config(attack_config, data_config, model_config, attack_batch_config=None, model_batch_config=None):
    config = dict()
    params_data = importlib.import_module('preset_config.data.' + data_config)
    params_model = importlib.import_module('preset_config.model.' + model_config)
    ...
    if attack_config is not None:
        params_attack = importlib.import_module('preset_config.attack.'+attack_config)
        attacker_list = dict()
    ...
    if attack_batch_config is not None:
        params_attack_batch = importlib.import_module('preset_config.batch.attack.' + attack_batch_config)
        config.update(params_attack_batch.config)

    if model_batch_config is not None:
        params_model_batch = importlib.import_module('preset_config.batch.model.' + model_batch_config)
