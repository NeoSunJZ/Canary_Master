...
class FGM():
    ...
    def attack(self, img, ori_label):
        ...
        y = torch.tensor([5]*batch_size,device=self.device)

        if self.attacktype == 'untargeted':
            img = fast_gradient_method(self.model, img, self.epsilon, np.inf, targeted=False) #非靶向 n_classes为int类型
        else:
            img = fast_gradient_method(self.model, img, self.epsilon, np.inf, targeted=True, y=y) #靶向 y带有真标签的张量
