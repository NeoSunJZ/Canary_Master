...
@sefi_component.config_params_handler(handler_target=ComponentType.ATTACK, name="CW",
                                      ...
                                      params={
                                          "n_classes": {"desc": "模型中类的数量", "type": "INT"},
                                          "lr": {"desc": "攻击算法的学习率", "type": "FLOAT", "df_v": "5e-3"},
                                          "confidence": {"desc": "对抗性示例的置信度，越高则生成对抗样本的l2越大，但误分类置信度越高", "type": "INT", "df_v": "0"},
                                          "clip_min": {"desc": "对抗样本像素上界(与模型相关)", "type": "FLOAT"},
                                          "clip_max": {"desc": "对抗样本像素下界(与模型相关)", "type": "FLOAT"},
                                          "initial_const": {"desc": "初始权衡常数，用于调整扰动大小的相对重要性和分类的置信度", "type": "FLOAT", "df_v": "1e-2"},
                                          "binary_search_steps": {"desc": "执行二分搜索时找到扰动范数和分类置信度之间的最佳权衡常数的次数", "type": "INT", "df_v": "5"},
                                          "max_iterations": {"desc": "最大迭代次数(整数)", "type": "INT", "df_v": "1000"},
...
class CW:
    def __init__(self, model, classes=1000, lr=5e-3, confidence=0, clip_min=0, clip_max=1, initial_const=1e-2,
                 binary_search_steps=5, max_iterations=1000,attacktype='UNTARGETED', tlabel=1):
        ...
        self.attack_type = attacktype  # 攻击类型：靶向 or 非靶向
