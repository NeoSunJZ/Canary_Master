...
class BoundaryAttack(MinimizationAttack):
    ...
    Args:
        ...
        source_step_convergence : Sets the threshold of the stop criterion:
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        if not is_adv.all():
            ...
            failed_index = np.argwhere(failed == 1).flatten()
        ...
        if len(failed_index) != 0:
            ...
            best_advs = ep.astensor(ep.stack(new_best_advs))
        ...
        if len(failed_index) != 0:
            ...
            final_best_advs = []
            counter = 0
            for idx in range(len(best_advs) + len(failed_index)):
                if idx in failed_index:
                    final_best_advs.append(originals[idx])
                else:
                    final_best_advs.append(best_advs[counter])
                    counter += 1

            final_best_advs = ep.stack(final_best_advs)
