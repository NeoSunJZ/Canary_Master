...
from foolbox.attacks.base import get_channel_axis
...
class GenAttack(FixedEpsilonAttack):
    ...
    def apply_noise(
        ...
        return ep.clip(x + noise, bounds.lower, bounds.upper)
        ...
        **kwargs: Any,
        ...
        for step in range(self.steps):
            ...
            for i in range(self.population):
                it = self.apply_noise(model.bounds,x, noise_pops[:, i], epsilon, channel_axis)
                logits = model(it)
            ...
            if is_adv.all():
                return restore_type(  # pragma: no cover
                    self.apply_noise(model.bounds,x, elite_noise, epsilon, channel_axis)
                )
        ...
        return restore_type(self.apply_noise(model.bounds,x, elite_noise, epsilon, channel_axis))
