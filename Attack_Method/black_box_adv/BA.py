...
@sefi_component.config_params_handler(handler_target=ComponentType.ATTACK, name="BA",
                                      ...
                                      params={
                                          ...
                                          "init_attack": {"desc": "攻击用来寻找起点 仅当starting_points为None时才使用", "df_v": "None"},
                                          "steps": {"desc": "要运行的最大步骤数，可能会在此之前收敛并停止", "type": "INT", "df_v": "25000"},
                                          "spherical_step": {"desc": "正交（球形）补偿的初始步长", "type": "FLOAT", "df_v": "0.01"},
                                          "source_step": {"desc": "迈向目标的步骤的初始步长", "type": "FLOAT", "df_v": "0.01"},
                                          "source_step_convergance": {"desc": "设置停止条件的阈值，若在攻击期间source_step小于此值，则攻击已收敛并将停止", "type": "FLOAT", "df_v": "1e-07"},
                                          "step_adaptation": {"desc": "步长乘以或除以的因子", "type": "FLOAT", "df_V": "1.5"},
                                          "tensorboard": {"desc": "TensorBoard摘要的日志目录(禁用(False)/运行/CURRENT_DATETIME_HOSTNAME(Nnoe))", "type": "STR", "df_v": "False"},
                                          "update_stats_every_k": {"type": "INT", "df_v": "10"}})
class BA():
    def __init__(self, model, epsilon=0.03, attacktype='UNTARGETED', tlabel=1, init_attack=None, steps=25000,
        ...
        self.tensorboard = tensorboard  # TensorBoard摘要的日志目录(禁用(False)/运行/CURRENT_DATETIME_HOSTNAME(Nnoe))
