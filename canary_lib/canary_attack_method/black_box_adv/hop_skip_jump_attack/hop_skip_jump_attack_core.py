    ...
    def run(
        ...
        **kwargs: Any,
        ...
        if not is_adv.all():
            ...
            if starting_points is None:
                print(f"init_attack failed for {failed} of {len(is_adv)} inputs")
            else:
                print(f"{failed} of {len(is_adv)} starting_points are not adversarial")
        del starting_points
        ...
        if len(failed_index) != 0:
            ...
            new_x_advs = []
            new_criterion = []
            for idx in range(len(x_advs)):
                if idx not in failed_index:
                    new_x_advs.append(restore_type(x_advs[idx]))
            ...
            del originals, x_advs, criterion, is_adversarial
            criterion = ep.astensor(ep.stack(new_criterion))
            criterion = get_criterion(criterion)
            is_adversarial = get_is_adversarial(criterion, model)
            originals = ep.astensor(ep.stack(new_originals))
            x_advs = ep.astensor(ep.stack(new_x_advs))
        ...
        for step in range(self.steps):
            ...
            x_advs = restore_type(x_advs)

            if len(failed_index) != 0:
                # restore failed inputs
                final_x_advs = []
                counter = 0
                for idx in range(len(x_advs) + len(failed_index)):
                    if idx in failed_index:
                        final_x_advs.append(originals[idx])
                    else:
                        final_x_advs.append(x_advs[counter])
                        counter += 1

                final_x_advs = ep.stack(final_x_advs)
                return final_x_advs
            else:
                return x_advs
